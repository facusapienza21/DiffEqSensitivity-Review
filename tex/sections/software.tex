...
If well this methods are implemented, in principle, independent of the differential equation and numerical solver involved. 
However, special consideration needs to be taken into account when using automatic differentiation thought a numerical solver, since the computed gradient can be exact but numerically unstable. 
Consider the following example of ordinary differential equation
\begin{align}
    \frac{du_1}{dt} &= a u_1 - u_1 u_2 & \quad u_1(0) = 1 \\
    \frac{du_2}{dt} &= - a u_2 + u_1 u_2 & \quad u_2(0) = 1.
\end{align}
when $a=1$, the solutions are given by $u_1(t) \equiv u_2(t) \equiv 0$. 
An adaptive ODE solver will increase the stepsize in each iteration, which will lead to numerical errors in the gradient calculation unless special care is taken to control the stepsize based on both the sensitivity of the solution for $u(t)$ and its gradients. 
